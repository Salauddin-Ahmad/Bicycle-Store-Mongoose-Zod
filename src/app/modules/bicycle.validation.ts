...
export const BicycleValidation = z.object({
  ...
  type: z.enum(Object.values(TypeInterface) as [TypeInterface, ...TypeInterface[]], { message: 'Invalid bicycle type', }),  
  description: z.string().min(1, { message: 'Description is required' }),
  quantity: z.number().min(1, { message: 'Quantity must be at least 1' }),
  inStock: z.boolean({ required_error: 'Stock status is required' }),
  createdAt: z.string(), // Optional as it will be set by the database
  updatedAt: z.string(), // Optional as it will be set by the database
});
